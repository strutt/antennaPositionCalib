...
//     sumGrads += myFit[0]*myFit[0];
  ...
  for(unsigned int vert = 0; vert < MAX_ANTENNAS; ++vert){
    
    mean = getMean(VerticalAntDeltaT, vert, countVertical[vert]);
    sumMeanVert += mean*mean;
    
//     vector<double> myFit = leastSquares(phiAngleArrayLoop, deltaTArrayLoop, count8-1);	
//     sumGrads += myFit[0]*myFit[0];
    delete [] VerticalAntPhiWave[vert];
    delete [] VerticalAntDeltaT[vert];    
  }
