...
void runQuickOptAllAnt() {
  ...
  gSystem->CompileMacro("quickOptAllAnt.C","k");
  ...
  myMin->SetObjectFit(quickOptAllAnt);
...
//   myMin->mnexcm("SET EPS", eps, 1, ierflg);
  ...
  for(int y = 0; y <MAX_ANTENNAS; y++){
    ...
    char name[30];
    sprintf(name,"r%d",y);
    myMin->DefineParameter(y, name, startValR[y], stepSize, -0.15, 0.15);
    sprintf(name,"z%d",y);
    myMin->DefineParameter(y+MAX_ANTENNAS, name, startValZ[y], stepSize, -0.1, 0.1);
    sprintf(name,"phi%d",y);
    myMin->DefineParameter(y+MAX_ANTENNAS*2, name, startValPhi[y], stepSize, -0.15, 0.15);
...
//   myMin->SetPrintLevel(-1);
  ...
  ofstream newfile(Form("newLindaNumbersCableDelays_ALLANTENNAS_RMS_GRAD_QUICK2_NEWLIM2_%s.txt", buffer));
...
void iHateRoot(Int_t& npar, Double_t* gin,
  ...
  double diffErr = quickOptAllAnt(par, eventNumberIndex, thetaWaveIndex, phiWaveIndex, antIndex1, antIndex2, maxCorrTimeIndex, adjacent);
