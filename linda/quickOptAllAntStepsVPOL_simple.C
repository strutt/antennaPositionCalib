...
double quickOptAllAntStepsVPOL_simple(double *par, double *eventNumberIndex, double *thetaWaveIndex, double *phiWaveIndex, int *antIndex1, int *antIndex2, double *maxCorrTimeIndex, bool *adjacent);
...
double quickOptAllAntStepsVPOL_simple(double *par, double *eventNumberIndex, double *thetaWaveIndex, double *phiWaveIndex, int *antIndex1, int *antIndex2, double *maxCorrTimeIndex, bool *adjacent){
  ...
  for(unsigned int i = 0; i<MAX_ANTENNAS;++i){
    deltaR[i]=par[2];
    deltaZ[i]=(i<16)*par[1]+(i>15)*(-par[1]);
    deltaPhi[i]=0;
    deltaCableDelays[i] = (i>15)*par[0] + (i<16)*0.;
...
void fillArrays(double *eventNumberIndex, double *thetaWaveIndex, double *phiWaveIndex, int *antIndex1, int *antIndex2, double *maxCorrTimeIndex, bool *adjacent){
 char headerName[FILENAME_MAX];
  ...
  for (unsigned int run=154;run<162;++run){
    ...
    sprintf(corName, "/unix/anita3/linda/corTrees/corRun_NEW12_VPOL_%d.root", run);
