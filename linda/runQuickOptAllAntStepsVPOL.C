...
void runQuickOptAllAntStepsVPOL() {
...
//   myMin->SetPrintLevel(-1);
  ...
  cout << " ################second half step done #############" << endl;
...
//   myMin->SetPrintLevel(-1);
  ...
  ofstream newfile(Form("RETEST/newLindaNumbers_4steps_VPOL_10kVSeavey_NEW14_onlyMean_%s.txt", buffer));
  for(int u = 0; u <MAX_ANTENNAS; u++){
