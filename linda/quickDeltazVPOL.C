...
double quickDeltazVPOL(double *par, double *eventNumberIndex, double *thetaWaveIndex, double *phiWaveIndex, int *antIndex1, int *antIndex2, double *maxCorrTimeIndex, bool *adjacent);
...
double quickDeltazVPOL(double *par, double *eventNumberIndex, double *thetaWaveIndex, double *phiWaveIndex, int *antIndex1, int *antIndex2, double *maxCorrTimeIndex, bool *adjacent){
  ...
  double temp;
  //   ifstream myInput(Form("/home/lindac/ANITA/Software/EventCorrelator/macros/lindaMacros/newLindaNumbers_4steps_2015_10_13_time_14_30_54.txt"));
  // if (myInput.is_open()){
  //   for (int i=0;i<48;i++){
  //     myInput >> temp >> deltaR[i] >> deltaZ[i] >> deltaPhi[i] >> deltaCableDelays[i];
  //     // cout << antArray[i] << " " << deltaCableDelays[i] << endl;
  //   }
  // }
  ...
  for(unsigned int i = 0; i<MAX_ANTENNAS;++i){
    // deltaR[i]+=par[0];;
    deltaZ[i]= (i<16)*par[0] + (i>15)*(-par[0]);
    // deltaPhi[i]+=par[2];//(i<16)*par[1] + (i>15)*(-par[1]);
     deltaCableDelays[i] = (i>15)*(par[1]);
  ...
  while(antIndex1[entry]!=-999) {
    //    std::cout << entry << "         " <<   antIndex1[entry]<< "      \r" << flush;    
...
void fillArrays(double *eventNumberIndex, double *thetaWaveIndex, double *phiWaveIndex, int *antIndex1, int *antIndex2, double *maxCorrTimeIndex, bool *adjacent){
 char headerName[FILENAME_MAX];
  ...
  for (unsigned int run=154;run<161;++run){
    ...
    sprintf(corName, "/unix/anita3/linda/corTrees/corRun_NEW6_VPOL_%d.root", run);
