...
double quickOptAllAntStepsLDBHPOL(double *par, double *eventNumberIndex, double *thetaWaveIndex, double *phiWaveIndex, int *antIndex1, int *antIndex2, double *maxCorrTimeIndex, bool *adjacent);
...
void fillArrays(double *eventNumberIndex, double *thetaWaveIndex, double *phiWaveIndex, int *antIndex1, int *antIndex2, double *maxCorrTimeIndex, bool *adjacent){
 char headerName[FILENAME_MAX];
  ...
  for (unsigned int run=151;run<153;++run){
  ...
  if (pol == AnitaPol::kVertical){
    ...
    sourceLat = - (77 + (51.23017/60)); // OLD NUMBERS
    sourceLon = +(167 + (12.16908/60));// OLD NUMBERS
    sourceAlt = 0;
   // sourceLat = - (79 + (27.94097/60));
   //  sourceLon = -(112 + (6.76208/60));
   //  sourceAlt = 1819.62;
