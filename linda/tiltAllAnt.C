...
double tiltAllAnt(double *par){
  ...
  for (unsigned int run=331;run<356;++run){
    ...
    sprintf(corName, "/home/lindac/ANITA/Software/EventCorrelator/macros/corTrees/corRun_NEW12_VPOL_%d.root", run);
...
//     VerticalAntPhiWave[i] = new double[BIGNUMBER];
  ...
  for (unsigned int ant=0;ant<MAX_ANTENNAS;++ant){
    ...
    deltaR[ant]= 0;//fGeomTool->getAntR(ant, pol)*TMath::Sin(tiltPos)*TMath::Cos(tilt) - fGeomTool->getAntR(ant, pol);
    deltaZ[ant]= 0;//fGeomTool->getAntR(ant, pol)*TMath::Sin(tiltPos)*TMath::Sin(tilt);
    deltaPhi[ant]= 0;//TMath::ACos(TMath::Sin(meanPhi[ant])*TMath::Cos(tilt))- meanPhi[ant];
    deltaCableDelays[ant] = par[0];
...
//     vector<double> myFit = leastSquares(phiAngleArrayLoop, deltaTArrayLoop, count8-1);	
//     sumGrads += myFit[0]*myFit[0];
//     delete [] VerticalAntPhiWave[vert];
...
//   delete [] VerticalAntPhiWave;
  ...
  cout << sumMeanAdj << " " << sumMeanVert << endl;
