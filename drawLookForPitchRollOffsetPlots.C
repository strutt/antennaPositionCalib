void drawLookForPitchRollOffsetPlots(){
  ...
  TH2D* hPhiRmsVals = new TH2D("hPhiRmsVals",
				 "RMS of #delta#phi distributions; Pitch offset (degrees); Roll offset (degrees); RMS (Degrees)",
  ...
  for(Int_t pitchInd=0; pitchInd < numPitches; pitchInd++){
    for(Int_t rollInd=0; rollInd < numRolls; rollInd++){
      ...
      // TString command = TString::Format("deltaPhiDegs[%d][%d]:zoomPhiDeg>>", pitchInd, rollInd);
      // command += histName;
      TString command = TString::Format("deltaPhiDegs[%d][%d]>>", pitchInd, rollInd);
      command += histName;
      ...
      hPhiRmsVals->SetBinContent(pitchInd+1, rollInd+1, h->GetRMS());
      p++;
    }
  }
