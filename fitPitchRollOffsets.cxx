...
int main(int argc, char *argv[])
{
  ...
  hDeltaPhiDeg->GetYaxis()->SetTitle("#delta#theta (Degrees)");
  ...
  Double_t mean=0;
  for(Int_t binx=1; binx<=hDeltaThetaDeg_pfx->GetNbinsX(); binx++){
    mean += hDeltaThetaDeg_pfx->GetBinContent(binx);
  }
  std::cout << mean/hDeltaThetaDeg_pfx->GetNbinsX() << std::endl;
  ...
  for(Int_t headInd=0; headInd < numPhiDegBins; headInd++){
    ...
    // std::cout << headInd << "\t" << usefulPat.heading << "\t" << phiExpected << "\t" << deltaThetaMeasured << "\t"
    // 	      << newDeltaTheta << std::endl;

    // Want newExpected - measured
    // (OldExpected - measured) - (OldExpected - newExpected) = newExpected - measured
    // Double_t diff = deltaThetaMeasured - newDeltaTheta;
    // Double_t diffSquared = diff*diff;
  ...
  // for(Int_t pitchInd=minPitchInd; pitchInd<maxPitchInd; pitchInd++){
  //   for(Int_t rollInd=minRollInd; rollInd<maxRollInd; rollInd++){
  //     usefulPat.pitch = pitchInd * deltaPitch;
  //     usefulPat.roll = rollInd * deltaRoll;
  //     Double_t thetaExpected, phiExpected;
  //     usefulPat.getThetaAndPhiWaveWaisDivide(thetaExpected, phiExpected);
  //     thetaExpected *= -1*TMath::RadToDeg();
  //     phiExpected *= TMath::RadToDeg();
  //     // std::cout << thetaExpected << "\t" << phiExpected << std::endl;
  //     // std::cout << thetaExpected - thetaExpected0 << std::endl;
  //     if(pitchInd==minPitchInd){
  // 	hDeltaRoll->SetBinContent(rollInd - minRollInd +1, thetaExpected - thetaExpected0);
  //     }
  //     if(rollInd==minRollInd){
  // 	hDeltaPitch->SetBinContent(pitchInd - minPitchInd +1, thetaExpected - thetaExpected0);
  //     }
  //   }
  // }
