...
void plotFilterPlotsForThesis(){

  TChain* cPhaseLdb = new TChain("angResTree");
  TChain* cPhaseWais = new TChain("angResTree");

  cPhaseWais->Add("filterOns/phaseCenter/generateAngularResolutionTreePlots_*");
  cPhaseLdb->Add("extremeFilter/phaseCenter/generateAngularResolutionTreeVPOLPlots_*");
  ...
  TH2D* hDeltaPhiDegPhaseWais3 = new TH2D("hDeltaPhiDegPhaseWais3", "HPol #delta#phi_{WAIS}; #delta#phi_{WAIS} (Degrees); #delta#phi (Degrees); Events per bin", nBinsPhi, 0, 360, phiRange/dPhi, -phiRange/2 - dPhi/2, phiRange/2 - dPhi/2);
  ...
  TH2D* hDeltaThetaDegPhaseWais3 = new TH2D("hDeltaThetaDegPhaseWais3", "HPol #delta#theta_{WAIS} Fitted PhaseWais Centre Antenna Positions; #delta#theta_{WAIS} (Degrees); #delta#theta (Degrees); Events per bin", nBinsPhi, 0, 360, thetaRange/dTheta, -thetaRange/2 - dTheta/2, thetaRange/2 - dTheta/2);
  ...
  TH2D* hDeltaPhiDegPhaseLdb3 = new TH2D("hDeltaPhiDegPhaseLdb3", "HPol #delta#phi_{WAIS} PhaseLdbgrammetry Antenna Positions; #delta#phi_{WAIS} (Degrees); #delta#phi (Degrees); Events per bin", nBinsPhi, 0, 360, phiRange/dPhi, -phiRange/2 - dPhi/2, phiRange/2 - dPhi/2);
  ...
  TH2D* hDeltaThetaDegPhaseLdb3 = new TH2D("hDeltaThetaDegPhaseLdb3", "HPol #delta#theta_{WAIS} PhaseLdbgrammetry Antenna Positions; #delta#theta_{WAIS} (Degrees); #delta#theta (Degrees); Events per bin", nBinsPhi, 0, 360, thetaRange/dTheta, -thetaRange/2 - dTheta/2, thetaRange/2 - dTheta/2);
